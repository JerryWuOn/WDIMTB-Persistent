...
export default function Information() {
  ...
  const toggleCollapse = (index) => {
    ...
    // Ensure scroll happens after expanding the section
    setTimeout(() => {
      const element = document.getElementById(`content-${index}`);
      if (element) {
        element.scrollIntoView({ behavior: "smooth", block: "center" });
      }
    }, 100); // slight delay to ensure the content has expanded
  };
          ...
          {[...Array(11)].map((_, index) => (
            <li key={index} className={styles.listItem}>
              <button
                ...
                aria-controls={`content-${index}`} // Associates button with its content
              >
                {/* Add the questions dynamically */}
                {getQuestion(index)}
              </button>
              {openIndex === index && (
                <div
  ...
  // Function to return questions based on index
  ...
  // Function to return answers based on index
